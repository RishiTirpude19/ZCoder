...
function SolutionReviews() {
  ...
  const renderStars = (rating) =>
    Array.from({ length: 5 }, (_, i) => (
      <Star
        key={i}
        className={`h-4 w-4 ${i < rating ? 'text-yellow-400 fill-current' : 'text-slate-500'}`}
      />
    ));

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      {/* Hero Section (matches ProblemReviews) */}
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-center">
              ...
              <p className="text-slate-400">See what others think about this solution</p>
            </div>
      ...
      {/* Reviews Section (matches ProblemReviews) */}
      <div className="max-w-6xl mx-auto px-6 pb-12">
        {reviews.length === 0 ? (
          <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-12 text-center">
            <p className="text-slate-400 text-lg">No reviews found for this solution.</p>
          </div>
        ) : (
